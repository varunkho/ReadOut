...
public static IntPtr[] FindDescendantWindows(IntPtr hParent, string dClass, bool directChildOnly = false)
        {
            List<IntPtr> wnds = new List<IntPtr>();
            EnumChildWindows(hParent, delegate(IntPtr hwnd, IntPtr lParam)
            {
                if (ClassName(hwnd).Equals(dClass, StringComparison.OrdinalIgnoreCase)
                    && (!directChildOnly || GetParent(hwnd) == hParent))
                    wnds.Add(hwnd);
                return 1;
                }, new IntPtr(0));
                        return wnds.ToArray();
...
public static void SayFindDescendantWindows(IntPtr hParent, bool directChildOnly = false)
{
        List<IntPtr> wnds = new List<IntPtr>();
    EnumChildWindows(hParent, delegate(IntPtr hwnd, IntPtr lParam)
    {
        if (!directChildOnly || GetParent(hwnd) == hParent)
        wnds.Add(hwnd);
                    return 1;
    }, new IntPtr(0));
            }
